...
function preferredMinWidth() {
    ...
    if (!tasks.vertical && !tasks.iconsOnly) {
        width += (units.smallSpacing * 2) + (theme.mSize(theme.defaultFont).width * 30 * tasksModel.count);
        width = Math.min(width, (units.smallSpacing * 2) + theme.mSize(theme.defaultFont).width * 90);
    }
